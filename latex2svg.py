...
def latex2svg(code, params=default_params, working_directory=None):
    ...
    with open(os.path.join(working_directory, filename + '.tex'), 'w') as f:
        f.write(document)

    # Run LaTeX and create DVI file
    try:
        ret = subprocess.run(shlex.split(params['latex_cmd']+' '+filename+'.tex'),
    ...
    # Convert DVI to SVG
    try:
        ret = subprocess.run(shlex.split(params['dvisvgm_cmd']+' '+filename+'.dvi'),
                             stdout=subprocess.PIPE, stderr=subprocess.PIPE,
    ...
    with open(os.path.join(working_directory, filename + '.svg'), 'r') as f:
        svg = f.read()
